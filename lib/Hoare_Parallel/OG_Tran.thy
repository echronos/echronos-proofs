...
| AnnBasic:  "s' \<in> f s \<Longrightarrow> (Some (AnnBasic r f), s) -1\<rightarrow> (None, s')"
...
| Basic:  "s' \<in> f s \<Longrightarrow> (Basic f, s) -P1\<rightarrow> (Parallel [], s')"
...
abbreviation (input) ID :: "'a \<Rightarrow> 'a set" where
  "ID \<equiv> \<lambda>s. {s}"
